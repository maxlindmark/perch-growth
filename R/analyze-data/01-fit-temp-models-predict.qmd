...
nd |> 
  ...
  geom_line() +
  labs(x = "Yearday", y = "Year", color = "Predicted SST (°C)", fill = "Predicted SST (°C)") + 
  geom_vline(xintercept = c(yday("2000-05-10"), yday("2000-10-15"))) + 
  geom_hline(yintercept = 10)

gs_min <- yday("2000-05-10")
gs_max <- yday("2000-10-15")
```
...
preds <- nd |> 
  filter(growth_dat == "Y" & yday %in% c(gs_min:gs_min) & source == "logger") |> 
  group_by(area, year) |> 
...
topt <- 17.5 # Overall t_opt from 02-fit-vbge.qmd! Update if needed
