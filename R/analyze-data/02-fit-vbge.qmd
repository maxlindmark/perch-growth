...
```{r}
#| cache=FALSE
pkgs <- c("tidyverse", "tidylog", "rTPC", "nls.multstart", "broom", "RColorBrewer", "viridis", "minpack.lm")
...
nareas <- length(unique(preds$area))
...
#   ungroup() |> 
#   mutate(area = fct_reorder(area, as.integer(mean_temp)))
# 
# preds |> group_by(area) |> summarise(m = mean(temp)) |> arrange(m)
# 
# VBG <- VBG |> 
#   group_by(area) |> 
#   mutate(mean_temp = mean(temp)) |> 
...
#   mutate(area = fct_reorder(area, as.integer(mean_temp)))
# 
# VBG |> group_by(area) |> summarise(m = mean(temp)) |> arrange(m)

# Check the temperature model script!
