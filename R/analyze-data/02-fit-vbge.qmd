...
n=10000
hist(rnorm(0.4, 0.4, n=n), main = "rtref")
hist(rnorm(1.4, 0.6, n=n), main = "e") 
hist(rnorm(2.1, 1.5, n=n), main = "eh")
hist(rnorm(7.5, 4, n=n), main = "hh")
...
fit_prior <- brm(
  ...
  seed = 9,
  prior = c(
    prior(normal(0.4, 0.4), nlpar = "rtref", lb = 0),
    prior(normal(1.4, 0.6), nlpar = "e"),
    prior(normal(2.1, 1.5), nlpar = "eh"),
    prior(normal(7.5, 4), nlpar = "th", lb = 0)
  )
)
...
dat |>
  ...
  mutate(rtref = 0.27, e = 0.7, eh = 1.8, th = 11.8) |> 
...
dat |>
  ...
  mutate(rtref = 0.4391, e = 1.4258, eh = 2.1066, th = 7.5369) |> # from summary(fit_all)
  mutate(man_pred = (rtref * exp(e/bk * (1/tref - 1/(temp + 273.15)))) / (1 + exp(eh/bk * (1/(th + 273.15) - 1/(temp + 273.15))))) |> 
  ggplot(aes(temp, y = .epred)) +
  stat_lineribbon(.width = c(0.95), alpha = 0.3, color = "black", fill = "black") + 
  geom_line(aes(y = man_pred), color = "tomato3", linetype = 2) + 
...
fitbs |>
  as_draws_df() |>
  dplyr::select( b_e_Intercept, b_eh_Intercept, b_th_Intercept) |> 
  rename(e = b_e_Intercept,
         eh = b_eh_Intercept,
         th = b_th_Intercept) |> 
  mutate(kb = 8.62e-05) |> 
  ungroup() |> 
  mutate(t_opt = (eh*th) / (eh + kb*th*log((eh/e)-1))) |> 
  drop_na(t_opt) |> 
  summarise(lwr_t_opt = quantile(t_opt, probs = 0.1),
            median_t_opt = median(t_opt),
            upr_t_opt = quantile(t_opt, probs = 0.9))
...
ggplot(nd_brms, aes(temp, y = pred_pop, linetype = "mixed brms model")) +
  geom_line() + 
  geom_line(data = pred_all, aes(temp, .fitted, linetype = "nls"), linewidth = 1, inherit.aes = FALSE) +
  geom_line(data = nd_brms, aes(temp, pred_glob, linetype = "global brms model"), linewidth = 1, inherit.aes = FALSE) +
  geom_point(data = dat, aes(temp, rate, color = factor(area, order$area)), size = 0.6) +
  scale_color_manual(values = colors, name = "Area") +
