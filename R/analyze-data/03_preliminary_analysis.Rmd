...
# plot temperature range in relation to growth data
d_min <- d |> 
  group_by(area) |> 
...
IVBG2 |>
  group_by(area) |> 
  summarise(min = min(cohort), max = max(cohort)) |> 
  arrange(min)
...
# sample size!
length(unique(IVBG2$ID))

d |>
  ungroup() |> 
  filter(ID %in% c(unique(IVBG2$ID))) |> 
  nrow()
...
p1 <- d |>
  ...
  guides(color = "none") +
  scale_color_viridis(discrete = TRUE, name = "Area") +
  labs(x = "Age (yrs)", y = "Length (mm)") +
  facet_wrap(~area)

ggsave("figures/VB_fits.pdf", width = 17, height = 15, units = "cm")

k <- IVBG2 |> 
  ggplot(aes(k, fill = area, color = area)) + 
  geom_density(alpha = 0.4, fill = NA) +  
  scale_color_viridis(discrete = TRUE, name = "Area") +
  #scale_fill_viridis(discrete = TRUE, name = "Area") +
...
linf <- IVBG2 |> 
  ggplot(aes(linf, fill = area, color = area)) + 
  geom_density(alpha = 0.4, fill = NA) +  
  scale_color_viridis(discrete = TRUE, name = "Area") +
  #scale_fill_viridis(discrete = TRUE, name = "Area") +
...
ggsave("figures/VB_par.pdf", width = 17, height = 17, units = "cm")
# 
# 
# IVBG2 |>
#   tidylog::drop_na(linf_se) |>
...
VBG <- VBG |> left_join(samplesize,by=c("cohort","area")) |> 
  mutate(keep = ifelse(area %in% c("FM", "BT") & cohort > 2004, "N", "Y")) |> 
  filter(keep == "Y")
...
gam_pred_temp <- read_csv("output/gam_predicted_temps.csv") |> dplyr::select(-`...1`)
...
pred_nls_fits$area2 <- factor(pred_nls_fits$area, levels = order$area)
VBG2$area2 <- factor(VBG2$area, levels = order$area)
colourCount = length(unique(order$area))
getPalette = colorRampPalette(brewer.pal(11, "RdBu"))

pred_nls_fits |>
  ggplot(.,aes(temp,.fitted,color=area2)) + 
  geom_point(aes(temp,k_median,color=area2),VBG2,size=0.6) + ## data
  geom_line(aes(temp,.fitted,group=area2),size=1) +
  geom_line(data = pred_all, aes(temp,.fitted),size=1, color = "black", linetype = 2) +
  scale_color_manual(values = getPalette(colourCount), name = "Area") +
  theme(plot.title=element_text(size=15,face="bold")) +
  theme(axis.text.x=element_text(angle=0)) +
  theme(axis.text=element_text(size=12),axis.title=element_text(size=15)) +
  guides(color=guide_legend(override.aes=list(size=1))) +
  scale_x_continuous(breaks=seq(-5,35,1)) +
  labs(x="Temperature (?C)",y="Growth rate coefficient") +
  NULL
...
VBG2 |>
  ...
  summarise(min = min(year), max = max(year)) |> 
  ungroup() |> 
  summarise(mean_ts = mean(max - min))
...
ggsave("figures/sharpe.pdf", width = 17, height = 15, units = "cm")

pred_nls_fits |>
  ggplot(.,aes(temp,.fitted)) + 
  geom_point(aes(temp,k_median),VBG2,size=0.6) + ## data
  geom_line(aes(temp,.fitted,group=area2),size=1) +
  geom_line(data = pred_all, aes(temp,.fitted),size=1, color = "black", linetype = 2) +
  theme(plot.title=element_text(size=15,face="bold")) +
  theme(axis.text.x=element_text(angle=0)) +
  ...
  scale_x_continuous(breaks=seq(-5,35,1)) +
  labs(x="Temperature (?C)",y="Growth rate coefficient") +
  facet_wrap(~factor(area, levels = rev(order$area))) + 
  NULL

test <- data.frame(temp = 7:23,
                   r = sharpeschoolhigh_1981(temp = 7:23, r_tref = 0.2243, e = 1.4636, eh=1.8599, th=10, tref=8))

pred_nls_fits |>
  ggplot(.,aes(temp,.fitted)) + 
  geom_point(aes(temp,k_median),VBG2,size=0.6) + ## data
  geom_line(data = pred_all, aes(temp,.fitted),size=1, color = "tomato", linetype = 2) +
  geom_smooth(data = pred_all, aes(temp,.fitted),size=1, color = "grey", linetype = 1) +
  stat_smooth(data = pred_all, aes(temp,.fitted),size=1, color = "blue", linetype = 3) +
  stat_smooth(data = pred_all, aes(temp,.fitted),size=1, linetype = 3,
              method = "gam", formula = y~s(x, k = 10), color = "green")  +
  geom_point(data = test, aes(temp, r), color = "pink", inherit.aes = FALSE) + 
  theme(plot.title=element_text(size=15,face="bold")) +
  theme(axis.text.x=element_text(angle=0)) +
  theme(axis.text=element_text(size=12),axis.title=element_text(size=15)) +
  guides(color=guide_legend(override.aes=list(size=1))) +
...
pred_nls_fits |> 
  ...
  stat_smooth(aes(temp,k_median, group = type), linetype = 1,
...
https://github.com/padpadpadpad/rTPC/blob/master/vignettes/bootstrapping_models.Rmd
