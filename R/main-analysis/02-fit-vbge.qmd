...
fits_ind <- ggplot(d2, aes(age_bc, length_mm, group = ID, color = ID)) +
  ...
  facet_wrap(~factor(area, order$area), ncol = 5)
...
p1 <- ggplot(no_adapt, aes(temp, growth_rate, color = pop)) + 
  ...
  labs(y = "Growth rate →", x = "Temperature →", color = "Site", 
       title = "No adaptation") +
  theme(legend.position.inside = c(0.5, 0.12),
...
adapt_3 <- tibble(temp = seq(15, 25, length.out = 500)) %>% 
  mutate(temp_sc = (temp - mean(VBG$temp) / sd(VBG$temp)),
         temp_sc_sq = temp_sc*temp_sc) %>% 
  mutate(growth_rate = par$intercept*0.725 + temp_sc*(par$b1 * 5) + temp_sc_sq*(par$b2*0.11),
         pop = "Warm",
...
p2 <- 
  ...
  geom_line(linewidth = 1.2, alpha = 0.8) + 
  labs(y = "Growth rate →", x = "Temperature →", title = "Local adaptation", 
...
ggsave(paste0(home, "/figures/concept.pdf"), width = 20, height = 8, units = "cm", device = cairo_pdf)
```
