...
colors <- colorRampPalette(brewer.pal(name = "RdYlBu", n = 10))(10)[c(2, 8)]

student <- data.frame(
  df_100 = rt(1000000, 100),
  df_5 = rt(1000000, 5)
) %>%
  pivot_longer(everything()) %>%
  mutate(name2 = ifelse(name == "df_5", "\u03BD = 5", "\u03BD = 100"))

p1 <- ggplot(student, aes(value, color = name2)) +
  ...
  coord_cartesian(xlim = c(-6, 6), expand = 0) +
...
p2 <- ggplot(student, aes(value, color = name)) +
  geom_density(alpha = 0.5, fill = NA) +
  theme(
    aspect.ratio = 2 / 3,
  ...
  labs(x = "Value", y = "") +
  guides(color = "none") +
  coord_cartesian(xlim = c(-6, 6), ylim = c(0, 0.02), expand = 0) +
...
ggsave(paste0(home, "/figures/supp/formas_student.pdf"), width = 15, height = 5, units = "cm", device = cairo_pdf)
```
...
nd <- data.frame(expand.grid(
  yday = seq(min(df$yday), max(df$yday), by = 1),
  area = unique(df$area),
  ...
  mutate(
    source_f = as.factor(source),
    year_f = as.factor(year)
  ) %>% # Left join in growth data column
...
```{r}
long_preds <- bind_rows(
  nd_area %>% filter(growth_dat == "Y") %>% mutate(model = "area-specific"),
...
for (i in unique(long_preds$area)) {
  ...
  print(
    ggplot(plotdat, aes(yday, pred, color = source, linetype = model)) +
      ...
      facet_wrap(~year) +
      geom_point(
        data = filter(d, area == i & growth_dat == "Y"), size = 0.2,
      ...
      labs(
        x = "Day of the year", y = "Predicted SST (°C)",
        title = paste("Site = ", i), color = "", linetype = ""
      )
  )
...
diff_pred <- wide_pred %>%
  mutate(
    logger_diff = logger_as - logger_full,
...
p1 <- preds_log %>%
  ...
  labs(x = "Site", y = "Manual residuals")
...
# Predict
ndbt <- data.frame(expand.grid(
  yday = seq(min(d$yday), max(d$yday), by = 1),
  year = seq(
    1980, # filter(gdat, area == "BT")$min,
    2004
  ), # filter(gdat, area == "BT")$min),
...
ggplot(ndbt, aes(yday, pred, color = source)) +
  ...
  facet_wrap(~year) +
  geom_point(
    data = filter(d, area == "BT" & year >= min(ndbt$year) & year <= max(ndbt$year)), size = 0.2,
    aes(yday, temp)
  ) +
  geom_line() +
...
# Plot without data
ggplot(
  filter(d, area == "BT" & year >= min(ndbt$year) & year <= max(ndbt$year)),
  aes(yday, temp, color = source)
) +
