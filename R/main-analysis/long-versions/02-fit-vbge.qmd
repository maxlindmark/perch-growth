...
#| cache: false
pkgs <- c(
  "tidyverse", "tidylog", "rTPC", "nls.multstart", "broom", "RColorBrewer", "ggridges",
...
ggplot() +
  geom_ribbon(data = VBG, aes(cohort, k_median,
    ymin = k_lwr, ymax = k_upr,
    fill = "All k's"
  ), alpha = 0.4) +
  geom_ribbon(data = VBG_filter, aes(cohort, k_median,
    ymin = k_lwr, ymax = k_upr,
    fill = "Filtered"
  ), alpha = 0.4) +
...
# ggsave(paste0(home, "/figures/for-talks/vb_fits.pdf" ), width = 14, height = 9, units = "cm")
...
# (k & coord_flip()) + (linf & coord_flip())
# ggsave(paste0(home, "/figures/for-talks/vb_pars.pdf" ), width = 16, height = 11, units = "cm")
```

## Test plotting growth coefficient

```{r}
# dat_test <- VBG %>%
#   mutate(A = 0.65*k_median*linf_median) # from Denderen et al 2020
#
# p1 <- ggplot(data = dat_test, aes(temp, A, color = factor(area, order$area))) +
...
#        y = "A") +
#   geom_smooth(data = dat_test, aes(temp, A), inherit.aes = FALSE,
#               method = "gam", formula = y~s(x, k=3))
#
# p1
#
# p2 <- ggplot(data = dat, aes(temp, rate, color = factor(area, order$area))) +
#   geom_point(size = 1, alpha = 0.8) +
#   coord_cartesian(expand = 0) +
#   scale_color_manual(values = colors, name = "Site") +
#   guides(fill = "none",
...
#   labs(x = "Temperature (°C)",
#        y = "A") +
#   geom_smooth(data = dat, aes(temp, rate), inherit.aes = FALSE,
#               method = "gam", formula = y~s(x, k=3))
#
# p2
#
...
prior <- c(
  prior(normal(0.8, 1), nlpar = "e", lb = 0),
  prior(normal(2, 1), nlpar = "eh", lb = 0),
  prior(normal(0.3, 0.5), nlpar = "rtref", lb = 0),
  prior(normal(10, 5), nlpar = "th", lb = 0)
)

# https://discourse.mc-stan.org/t/gamma-prior-parameters/18084/2
# prior_gamma <- c(prior(gamma(2, 1.5), nlpar = "e", lb = 0),
#                  prior(gamma(2, 1), nlpar = "eh", lb = 0),
#                  prior(gamma(2, 4), nlpar = "rtref", lb = 0),
#                  #prior(gamma(20, 2), nlpar = "th", lb = 0),
#                  prior(normal(10, 7), nlpar = "th", lb = 0))
#
# hist(rgamma(2, rate = 1.5, n=n), main = "e")
# hist(rgamma(2, rate = 1, n=n), main = "eh")
# hist(rgamma(2, rate = 4, n=n), main = "r_tref")
# #hist(rgamma(10, rate = 1, n=n), main = "th", xlim = c(5, 30))
...
# Compare with nls()
nls_ests <- nls_pooled_pars %>% pivot_longer(everything(), names_to = "parameter")
...
ggplot(dist, aes(value, fill = type)) +
  ...
  labs(fill = "") +
  geom_point(data = nls_ests, aes(x = value, y = 0), inherit.aes = FALSE, size = 5)
```
...
prior2 <- c(
  prior(normal(0.8, 1.5), nlpar = "e", lb = 0),
...
prior3 <- c(
  prior(normal(0.8, 3), nlpar = "e", lb = 0),
...
prior4 <- c(
  prior(normal(0.8, 4.5), nlpar = "e", lb = 0),
...
prior5 <- c(
  prior(normal(0.8, 6.75), nlpar = "e", lb = 0),
...
prior6 <- c(
  prior(normal(0.8, 10.125), nlpar = "e", lb = 0),
...
ggplot(dist_df %>% filter(parameter == "r_tref"), aes(value,
...
ggplot(dist_df %>% filter(parameter == "e"), aes(value,
  color = as.factor(iter),
  linetype = as.factor(type)
)) +
  geom_density(fill = NA) +
...
ggplot(dist_df %>% filter(parameter == "eh"), aes(value,
  color = as.factor(iter),
  linetype = as.factor(type)
)) +
  geom_density(fill = NA) +
...
ggplot(dist_df %>% filter(parameter == "th"), aes(value,
  color = as.factor(iter),
  linetype = as.factor(type)
)) +
  geom_density(fill = NA) +
...
ts <- dat %>%
  ...
  add_epred_draws(fit_global, re_formula = NA, ndraws = 8000) # fitbs or fitglobal?
...
dat %>%
  ...
  stat_lineribbon(
    data = glob, aes(temp, .epred), .width = c(0.9), inherit.aes = FALSE,
    fill = "black", color = "black", alpha = 0.1
  ) +
  geom_point(data = dat, aes(temp, rate, color = factor(area, order$area)), size = 1, alpha = 0.8) +
  stat_lineribbon(.width = c(0)) +
  stat_lineribbon(
    data = glob, aes(temp, .epred), .width = c(0), inherit.aes = FALSE,
...
dat %>%
  ...
  add_epred_draws(fitbs) %>%
  ungroup() %>%
  mutate(alpha = ifelse(area %in% c("BT", "FM"), "y", "n")) %>%
  ggplot(aes(temp, y = .epred, color = factor(area, order$area), alpha = alpha)) +
  stat_lineribbon(
    data = glob, aes(temp, .epred), .width = c(0.9), inherit.aes = FALSE,
    fill = "black", color = "black", alpha = 0.1
  ) +
  geom_point(
    data = dat %>% mutate(alpha = ifelse(area %in% c("BT", "FM"), "y", "n")),
    aes(temp, rate, color = factor(area, order$area), alpha = alpha),
    size = 1
  ) +
  stat_lineribbon(.width = c(0)) +
...
ggsave(paste0(home, "/figures/for-talks/pices_sharpe_school_bayes.pdf"), width = 14, height = 14, units = "cm")
```
...
# Compute quantiles across spaghetties
rect_dat <- data.frame(
  xmin = c(quantile(t_opt$temp, probs = c(0.05)), quantile(t_opt$temp, probs = c(0.1))),
...
ridge <- ggplot(t_opt_a, aes(temp, factor(area, levels = rev(order$area)), fill = factor(area, levels = rev(order$area)))) +
  ...
  geom_rect(
    data = rect_dat %>% filter(interval == "90%"), aes(xmin = xmin, xmax = xmax), ymin = -Inf, ymax = Inf,
    inherit.aes = FALSE, fill = "grey", alpha = 0.5
  ) +
  geom_rect(
    data = rect_dat %>% filter(interval == "80%"), aes(xmin = xmin, xmax = xmax), ymin = -Inf, ymax = Inf,
    inherit.aes = FALSE, fill = "grey", alpha = 0.5
  ) +
  geom_density_ridges(
    alpha = 0.7, color = NA, scale = 1,
  ...
  guides(fill = "none") +
  geom_errorbar(
    data = t_opt_a_sum, aes(xmin = lwr, xmax = upr, y = area, color = factor(area, levels = rev(order$area))), linewidth = 1,
...
p1 <- ggplot(area_pred_brms, aes(temp,
  y = .epred, color = factor(area, order$area),
...
ggplot(nd_brms, aes(temp, y = pred_pop, color = "mixed brms model")) +
  ...
  guides(
    fill = "none",
    color = guide_legend(
      ncol = 1, reverse = TRUE, title.position = "top", title.hjust = 0.5,
      override.aes = list(size = 1)
    ),
...
brms_pars <- fitbs %>%
  ...
  rename(parameter = .variable) %>%
  mutate(
    parameter = str_remove(parameter, "r_area__"),
    parameter = ifelse(parameter == "rtref", "r_tref", parameter)
  ) %>%
...
ggplot(area_pred_brms, aes(temp,
  y = .epred, color = factor(area, order$area),
  fill = factor(area, order$area)
)) +
  ...
  stat_lineribbon(
    data = area_spec_preds, .width = 0, alpha = 0.6,
...
ggplot(area_pred_brms %>% filter(area == "RA"), aes(temp,
  y = .epred, color = factor(area, order$area),
  fill = factor(area, order$area)
)) +
  stat_lineribbon(.width = c(0.95), alpha = 0.1, color = NA) +
  ...
  # add non-hierarchical model predictions
  stat_lineribbon(
    data = area_spec_preds %>% filter(area == "RA"), .width = 0, alpha = 0.6,
    aes(temp, y = .epred, color = factor(area, order$area), linetype = "non-hierarchial")
  ) +
  # add nls
