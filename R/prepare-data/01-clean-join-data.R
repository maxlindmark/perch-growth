...
BTAB_77_01 <- bind_rows(
  BTAB77, BTAB78, BTAB79, BTAB80, BTAB81, BTAB83,
...
BTAB_77_01 <- BTAB_77_01 |>
  separate(X1,
    sep = c(
      2, 4, 8, 10, 13, 15, 17, 18, 21, 22, 23, 26, 29, 32, 35, 38, 41, 44, 47, 50, 53, 56, 59,
      62, 65, 68, 71, 74, 77
    ),
    into = c(
      "bl", "area", "species", "red", "stn_nr", "catch_year", "cohort", "sex",
  ...
  mutate_at(c(
    "red", "stn_nr", "catch_year", "cohort", "sex",
...
BTAB_02_20 |>
  group_by(catch_year) |>
  distinct(Magnifikation) |>
  as.data.frame()
BTAB_02_20 |>
  group_by(catch_year) |>
  distinct(is.na(length_mm))
...
BT_77_20 |>
  ...
  summarise(n = n()) |>
  ggplot(., aes(catch_year, n, fill = source)) +
...
BSAB_91_01 <- bind_rows(
  BSAB91, BSAB92, BSAB93, BSAB94, BSAB95, BSAB96,
...
BSAB_03_06 <- BSAB_03_06 |>
  mutate_at(c(
    "År1", "År2", "År3", "År4", "År5", "År6", "År7", "År8", "År9", "År10", "År11", "År12",
    "År13", "År14", "År15", "År16", "År17", "År18", "År19", "År20"
  ), as.numeric)
...
FBAB_77_01 <- bind_rows(
  FBAB77, FBAB78, FBAB79, FBAB80, FBAB81, FBAB83,
...
FBAB_02_20 <- bind_rows(
  FBabbo_07_20_gear64_aug,
  FBabbo2002,
  FBabbo2003,
  FBabbo2004,
  FBabbo2005,
...
FM_71_01 <- bind_rows(
  FMAB70, FMAB71, FMAB72, FMAB73, FMAB74, FMAB75, FMAB76, FMAB77,
...
MUAB_91_01 <- bind_rows(
  MUAB91, MUAB92, MUAB93, MUAB94, MUAB95, MUAB96,
...
RA_90_01 <- RA_90_01 |>
  separate(X1,
    ...
    into = c(
      "bl", "area", "species", "gear", "stn_nr", "catch_year", "cohort", "sex",
      "sample_nr", "number_rows", "growth_catch_year", "final_length", "X1",
      "X2", "X3", "X4", "X5", "X6", "X7", "X8", "X9", "X10", "X11", "X12", "X13",
  ...
  mutate_at(c(
    "gear", "stn_nr", "catch_year", "cohort", "sex",
    "sample_nr", "number_rows", "final_length", "X1",
    "X2", "X3", "X4", "X5", "X6", "X7", "X8", "X9", "X10", "X11", "X12", "X13",
    "X14", "X15", "X16", "X17", "X18", "X19", "X20"
  ), as.numeric)
...
SIAB_63_08 <- SIAB_63_08 |> mutate(
  area = ifelse(SEK == 1, "SI_EK", SEK),
...
SIAB_07_19_tot <- SIAB_07_19_tot |>
  mutate_at(c(
    "Tillväxt..mm.år.1", "År2", "År3", "År4", "År5", "År6", "År7", "År8", "År9", "År10", "År11", "År12",
    "År13", "År14", "År15", "År16", "År17", "År18", "År19", "År20"
  ), as.numeric) |>
  rename("År1" = "Tillväxt..mm.år.1")
...
# Plot length-at-age
VI_95_02 <- VI_95_02 |>
  mutate(keep = ifelse(reading_no < 6 & length_mm > 400, "N", "Y")) |>
  filter(keep == "Y")
...
d <- d_full |>
  dplyr::select(
    length_mm, reading_no, age, age_ring, area, catch_year,
...
ggplot(d, aes(factor(sex), factor(sex2))) +
  geom_point()
...
# Add sex and age and gear to ID to make it unique by ID
d <- d |> mutate(ID = paste(ID, sex, age_catch, gear, sep = "."))
